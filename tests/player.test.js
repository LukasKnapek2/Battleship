...
test('Player has type "human" or "computer"', () => {
  ...
  expect(() => createPlayer("Invalid Player", "alien")).toThrow(
    'Invalid player type. Use "human" or "computer".'
  );
});
...
test("Player cannot attack invalid coordinates (e.g., already attacked or out of bounds)", () => {
  ...
  expect(() => player.attack(opponent, [1, 0])).toThrow(
    "Coordinates have already been attacked (hit ship segment)."
  );
  ...
  expect(() => player.attack(opponent, [5, 5])).toThrow(
    "Coordinates have already been attacked (miss)."
  );
  // Verify missed attacks count hasn't changed (or contains only one instance of [5,5])
  expect(
    opponent.gameboard
      .getMissedAttacks()
      .filter((c) => c[0] === 5 && c[1] === 5).length
  ).toBe(1);

  // Test out of bounds attacks
  expect(() => player.attack(opponent, [10, 10])).toThrow(
    "Attack coordinates are out of bounds."
  );
  expect(() => player.attack(opponent, [-1, -1])).toThrow(
    "Attack coordinates are out of bounds."
  );
  // Test invalid coordinate format
  expect(() => player.attack(opponent, [1])).toThrow(
    'Human players must provide valid [row, col] coordinates for attack.'
  );
});
...
test("Computer player can attack randomly", () => {
  ...
  if (attackResult === "hit") {
    ...
    expect(player.gameboard.getMissedAttacks()).toContainEqual(
      expect.any(Array)
    ); // Ensure coordinates are recorded
  }
});
    ...
    while (!attackAttempted) {
      try {
        ...
        if (result === "hit") {
          ...
          attackedCoord = player.gameboard
            .getBoard()
            .flat()
            .find(
              (cell) =>
                typeof cell === "object" &&
                cell.ship === ship &&
                ship.getHitSegments()[cell.segmentIndex]
            );
          ...
          // result === 'miss'
          attackedCoord =
            player.gameboard.getMissedAttacks()[
              player.gameboard.getMissedAttacks().length - 1
            ];
        }
    ...
    row.forEach((cell, cIdx) => {
      if (
        ...
        (typeof cell === "object" &&
          cell !== null &&
          cell.ship &&
          cell.ship.getHitsCount() > 0 &&
          cell.ship.getHitSegments()[cell.segmentIndex])
      ) {
...
test("throws error when ship placement is out of bounds", () => {
  ...
  expect(() => board.placeShip(ship, [0, 6], "horizontal")).toThrow(
    "Ship placement out of bounds."
  );
  expect(() => board.placeShip(ship, [6, 0], "vertical")).toThrow(
    "Ship placement out of bounds."
  );
  expect(() => board.placeShip(ship, [10, 0], "horizontal")).toThrow(
    "Ship placement out of bounds."
  );
});
...
test("throws error when ship placement collides with another ship", () => {
  ...
  expect(() => board.placeShip(ship2, [0, 1], "horizontal")).toThrow(
    "Ship collides with another ship or existing placement."
  );
  ...
  expect(() =>
    board.placeShip(shipToCollideVertically, [0, 0], "vertical")
  ).toThrow("Ship collides with another ship or existing placement.");

  const ship3 = createShip(3);
  board.placeShip(ship3, [5, 5], "vertical");
  expect(() => board.placeShip(ship2, [6, 5], "horizontal")).toThrow(
    "Ship collides with another ship or existing placement."
  );
});
